...
import 'Nodeicsesubject.dart';
import 'mpbuynow.dart';
...
class icsesubject extends StatefulWidget {
  @override
  _icsesubjectState createState() => _icsesubjectState();
}
int rindex;
class _icsesubjectState extends State<icsesubject> {
  List<Nodeicsesubject> _nodes=List<Nodeicsesubject>();
  Future <List<Nodeicsesubject>> getdata() async {
    var req = await http.get('http://ankushapp.iotans.in/api/get_subject_by_class/?boardname=ICSE&classname='+universaldata.SelectedClassname);
    // var nodes = {new Node("neeraj", "ok"),new Node("Radhe", "ok")};
    // print(universaldata.sname);
    List<Nodeicsesubject> nodes=List<Nodeicsesubject>();
    ...
    for (var nodeJson in nodesJson) {
      ...
      nodes.add(new Nodeicsesubject(Subjectid,
          BoardName,
          classname,
          SubjectName,
          Description,
